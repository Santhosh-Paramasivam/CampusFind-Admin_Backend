...
public class InstitutionSchemaController {
    ...
    @GetMapping("/get_schema")
    public ResponseEntity<Object> getInstitutionSchema(@RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader) throws ExecutionException, InterruptedException {
        String institutionId = institutionService.getInstitutionIdFromAuthToken(authHeader);
        InstitutionMemberSchema institutionMemberSchema = institutionSchemaService.getMemberSchemaFromInstitutionId(institutionId);

        return ServerResponse.respond(HttpStatus.ACCEPTED, institutionMemberSchema);
    }
}